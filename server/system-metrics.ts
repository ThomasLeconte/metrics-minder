    ...
    getDisksInfos() {
        ...
        return Promise.all([si.blockDevices(), si.fsSize(), si.diskLayout()]).then(([blockDevices, disks, disksLayout]) => {
            ...
            if(disksLayout && disksLayout.length > 0) {
                filteredBlockDevices = filteredBlockDevices
                    .filter(b => disksLayout.find(d => d.name === b.name || d.device === b.device));
            }

            if(filteredBlockDevices && filteredBlockDevices.length > 0) {
                ...
                if(_filteredDisks && _filteredDisks.length > 0) {
                    filteredDisks = _filteredDisks;
                }
            }
